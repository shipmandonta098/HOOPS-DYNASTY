...
export const ARCHETYPES = {
  PG: [
    { id: 'floor_general',  label: 'Floor General',   w: 3, d: { phy: -4, sho:  +2, ply: +14, def:  -3, reb:  -8 } },
    { id: 'scoring_guard',  label: 'Scoring Guard',   w: 3, d: { phy:  +1, sho: +13, ply:  +4, def:  -8, reb:  -9 } },
    { id: 'two_way_guard',  label: 'Two-Way Guard',   w: 2, d: { phy:  +4, sho:  +1, ply:  +6, def:  +9, reb:  -6 } },
    { id: 'burst_athlete',  label: 'Burst Athlete',   w: 2, d: { phy: +14, sho:  -9, ply:  +5, def:  +2, reb:  -5 } },
  ],
  SG: [
    { id: 'sharpshooter',   label: 'Sharpshooter',    w: 3, d: { phy:  -6, sho: +17, ply:  -2, def:  -7, reb:  -6 } },
    { id: 'three_and_d',    label: '3&D Wing',        w: 3, d: { phy:  +2, sho:  +8, ply: -10, def: +12, reb:  -3 } },
    { id: 'slasher',        label: 'Slasher',         w: 2, d: { phy: +13, sho:  -3, ply:  +2, def:  -2, reb:  -2 } },
    { id: 'combo_guard',    label: 'Combo Guard',     w: 2, d: { phy:  +1, sho:  +7, ply:  +8, def:  -6, reb:  -7 } },
  ],
  SF: [
    { id: 'wing_scorer',    label: 'Wing Scorer',     w: 3, d: { phy:  +5, sho: +12, ply:  +1, def:  -8, reb:  -6 } },
    { id: 'defensive_wing', label: 'Defensive Wing',  w: 3, d: { phy:  +6, sho:  -9, ply:  -5, def: +14, reb:  +3 } },
    { id: 'point_forward',  label: 'Point Forward',   w: 2, d: { phy:  -2, sho:  +3, ply: +14, def:  -4, reb:  +2 } },
    { id: 'finisher',       label: 'Athletic Finisher', w: 2, d: { phy: +15, sho: -10, ply:  -3, def:  +3, reb:  +5 } },
  ],
  PF: [
    { id: 'stretch_four',   label: 'Stretch Four',    w: 3, d: { phy:  -6, sho: +18, ply:  +2, def:  -7, reb:  -4 } },
    { id: 'defensive_big',  label: 'Defensive Big',   w: 3, d: { phy:  +5, sho: -13, ply:  -7, def: +13, reb: +10 } },
    { id: 'rim_runner',     label: 'Rim Runner',      w: 2, d: { phy: +14, sho: -14, ply:  -6, def:  +4, reb: +10 } },
    { id: 'face_up_four',   label: 'Face-Up Four',    w: 2, d: { phy:  +3, sho:  +8, ply:  +7, def:  -5, reb:  -2 } },
  ],
  C: [
    { id: 'rim_protector',  label: 'Rim Protector',   w: 4, d: { phy:  +6, sho: -20, ply: -12, def: +14, reb: +13 } },
    { id: 'post_hub',       label: 'Post Hub',        w: 2, d: { phy:  +4, sho:  -8, ply:  +9, def:  +2, reb:  +9 } },
    { id: 'stretch_five',   label: 'Stretch Five',    w: 2, d: { phy:  -5, sho: +19, ply:  +1, def:  -4, reb:  +2 } },
    { id: 'athletic_big',   label: 'Athletic Big',    w: 3, d: { phy: +15, sho: -15, ply:  -8, def:  +8, reb: +11 } },
  ],
};
 ...
 */
function buildAttributes(rng, scores) {
  const attrs = {};
  for (const g of ATTR_GROUPS) {
    const target = scores[g.key];
    for (const [attr] of g.parts) attrs[attr] = clamp(target + rng.gauss(0, 6.5), 20, 99);
    for (let i = 0; i < 3; i++) {
      const got = groupScore({ attributes: attrs }, g);
      const gap = target - got;
      if (gap === 0) break;
      for (const [attr] of g.parts) attrs[attr] = clamp(attrs[attr] + gap, 20, 99);
    }
...
export function makeRatedPlayer(rng, { target, position, age, potBias = 1 }) {
  ...
  const attributes = buildAttributes(rng, scores);
